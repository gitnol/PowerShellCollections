<#
.SYNOPSIS
    Funktionen zur Wegsuche in einem als JSON beschriebenen gerichteten Graphen.

.DESCRIPTION
    Baut aus einer JSON-Graphbeschreibung Nachschlagetabellen auf - ID zu
    QualifiedName und Space sowie umgekehrt - und sucht darin kuerzeste Wege
    per Breitensuche. Knoten lassen sich ueber ihre ID, ueber den
    QualifiedName oder ueber die Kombination Space/QualifiedName adressieren.

    Enthalten sind unter anderem Get-EdgeListWithSpaceAndQN, Find-ShortestPath,
    Find-ShortestPathByID und Get-ChildNodesInSameSpace.

.NOTES
    Codebeispiel ohne konkreten Betriebszweck, liegt deshalb unter snippets/.
