...
| Schicht | Technologie |
|---------|-------------|
| Orchestrierung | PowerShell 7.0+ (`New-Certificate.ps1`) |
...
```mermaid
flowchart TD
    A["New-Certificate.ps1<br />(Orchestrator)"] --> B{Modus?}
...
```mermaid
flowchart TD
    A["Set-VMwareCertificate.ps1"] --> B["vmware-config.json<br />(vCenter-Topologie)"]
    A --> C["CertificateResults[]<br />(Ausgabe von New-Certificate.ps1)"]
...
```
New-Certificate.ps1          → Orchestrierung: Config/CLI lesen, die gleichnamige Funktion aufrufen, PSCustomObject-Ergebnisse zurückgeben
Request-Certificate.ps1           → ADCS-Anfrage via certreq.exe, PFX-Export aus LocalMachine-Store, Store-Cleanup
Set-VMwareCertificate.ps1   → Phasiertes Cert-Deployment auf vCenter/ESXi via PowerCLI
...
```mermaid
flowchart LR
    A["New-Certificate.ps1"] -->|"PSCustomObject[]<br />HostnameFQDN, PfxPath,<br />PemPath, CaChainPath,<br />CertificatePath, PrivateKeyPath"| B["Set-VMwareCertificate.ps1"]
...
| Einzel-CA-Annahme | `Set-VMwareCertificate.ps1` lädt nur eine CA-Chain (die aus `CertificateResults[0]`); Warnung bei mehreren distinct Pfaden |
