<#
.SYNOPSIS
    Erzeugt aus der Online-Funktionsreferenz von MailStore PowerShell-Huellen
    fuer jede API-Methode.

.DESCRIPTION
    MailStore veroeffentlicht keine OpenAPI- oder Swagger-Beschreibung seiner
    Administrations-API. Dieses Skript laedt stattdessen die HTML-Seite der
    Funktionsreferenz, raeumt sie auf, liest Methodennamen und Parameter aus
    Ueberschriften und Tabellen und schreibt daraus fertige function-Bloecke.

    Das Ergebnis ist MailStoreApiFunctions.ps1.

.NOTES
    Der Generator haengt am HTML-Aufbau der Herstellerseite und ist
    entsprechend zerbrechlich. Er hat mindestens einmal einen leeren Parameter
    erzeugt, der die Zieldatei unparsbar machte - siehe
    Invoke-MailStoreApiScratch.ps1 und docs/BACKLOG.md. Die Ausgabe nach jedem
    Lauf gegen den PowerShell-Parser pruefen.
#>
...
function New-PowershellFunctionFromDefinition {
    ...
    $firstUppercasePosition = ($search.ToCharArray() | ForEach-Object {$_} | Where-Object {$_ -cmatch "[A-Z]"} | Select-Object -First 1)
