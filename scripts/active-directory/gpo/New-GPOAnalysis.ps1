<#
.SYNOPSIS
    Vergleicht bei allen GPOs den konfigurierten Status mit den tatsaechlich
    vorhandenen Einstellungen.

.DESCRIPTION
    Holt zu jeder Gruppenrichtlinie den XML-Report und prueft, ob der
    Benutzer- beziehungsweise Computerteil ueberhaupt Einstellungen enthaelt.
    Das deckt zwei haeufige Faelle auf: ein aktivierter Teil ohne jede
    Einstellung, der bei jeder Anmeldung unnoetig verarbeitet wird, und ein
    deaktivierter Teil mit Einstellungen, der wirkungslos ist, aber leicht mit
    einer aktiven Richtlinie verwechselt wird.

.NOTES
    Benoetigt das Modul GroupPolicy und Leserechte auf alle GPOs.
#>
