<#
.SYNOPSIS
    GUI zum Vergeben zeitlich begrenzter AD-Gruppenmitgliedschaften ueber das
    PAM-Feature (Stand v1).

.DESCRIPTION
    WinForms-Oberflaeche, die bestehende temporaere Mitgliedschaften auflistet
    und Auswahlfelder fuer Benutzer, Gruppe und Ablaufzeitpunkt bietet.
    Setzt voraus, dass das Privileged Access Management Feature in der
    Gesamtstruktur aktiviert ist; das Skript prueft das beim Start.

    Abgeloest durch Show-TemporaryGroupMembershipGui_v2.ps1. Nicht zu
    verwechseln mit Sync-TemporaryGroupMembershipFromCsv_v2/_v3.ps1 - das ist
    ein unbeaufsichtigter CSV-Stapellauf, kein interaktives Werkzeug.
    Siehe docs/BACKLOG.md.

.NOTES
    Das PAM-Feature laesst sich nicht mehr abschalten, sobald es in der
    Gesamtstruktur aktiviert wurde.
#>
